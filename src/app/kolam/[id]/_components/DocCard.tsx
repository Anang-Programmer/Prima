...
export function DocCard({ d, stage, popLabel, tab, setTab, onEditAbw, onEndCycle }: any) {
  return (
    <>
      <section className="relative overflow-hidden rounded-xl bg-white  ring-1 ring-slate-100 mb-4">
        {/* ============ TIPE & PROGRESS ============ */}
        <div className="flex items-center justify-between border-b border-slate-100 p-4">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">DOC</p>
            <h2 className="mt-0.5 text-2xl font-bold text-slate-800">{d.doc} hari</h2>
          </div>
          <div className="text-right">
            {d.cycle ? (
              <>
                <span className="inline-block rounded-full bg-[#E5F5F7] px-2.5 py-1 text-[10px] font-bold text-[#2ABFC8]">{stage}</span>
                <p className="mt-1 text-sm font-bold text-[#1C9098]">FCR {d.fcr.toFixed(2)}</p>
              </>
            ) : (
      ...
      {/* ============ METRIK UTAMA ============ */}
      <section className="grid grid-cols-4 divide-x divide-slate-100 rounded-xl bg-white py-3  mb-4">
        {[
      ...
      {d.doc >= TARGET_HARI && (
        <div className="mx-0 mt-0 mb-4 bg-[#CFE8EB] flex items-center justify-between px-4 py-4 rounded-b-none border-t-0 ">
          <span className="text-sm font-semibold text-slate-800">Sudah panen</span>
          <button onClick={onEndCycle} className="bg-[#2ABFC8] hover:bg-[#1C9098] text-white px-5 py-2 rounded-lg text-xs font-semibold  transition-colors">
            Akhiri Siklus
          </button>
        </div>
      )}
      ...
      <div className="flex border-b border-slate-200">
        {["Pakan", "Log Pakan", "Proyeksi", "Monitoring"].map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
